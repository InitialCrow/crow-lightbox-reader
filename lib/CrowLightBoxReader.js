...
var CrowLightBoxReader = function (_Component) {
  ...
  function CrowLightBoxReader(props) {
    ...
    _this.showPool = props.settings.showPool || true; //show thumbnail of pool in lightbox
    _this.showBtn = props.settings.showBtn || true; // show next and prev button
    _this.showCloseBtn = props.settings.showCloseBtn || true; // show close button
    _this.showDesc = props.settings.showDesc || true; // show text descripton on lightbox
    _this.showPagination = props.settings.showPagination || true; // show pagination of pool on lightbox
    _this.showLightBox = props.settings.showLightBox || false; //show lightbox
    _this.debug = props.settings.debug || false; // enable debug mode to show console msg
    //construct props
    _this.nodes = {
      thumb: [],
      desc: null
    ...
    value: function renderItem() {
      ...
      if (item.type === "video") {
        return _react2.default.createElement(
          ...
          this.renderBtn(),
          _react2.default.createElement('video', { width: item.width, height: item.height, src: item.src, autoPlay: true, controls: true }),
      ...
      if (item.type === "audio") {
        return _react2.default.createElement(
          ...
          _react2.default.createElement('audio', { className: 'audio', src: item.src, autoPlay: true, controls: true }),
          this.renderPagination()
        );
      }
      if (item.type === "image") {
    ...
    value: function renderPoolItemsThumb() {
      ...
      if (this.poolItems != "") {
        ...
        this.poolItems.map(function (item, i) {
          if (_this8.renderVideoItemThumb(item, i) !== undefined) {

            poolItems.push(_this8.renderVideoItemThumb(item, i));
          }
          if (_this8.renderImageItemThumb(item, i) !== undefined) {

            poolItems.push(_this8.renderImageItemThumb(item, i));
          }
          if (_this8.renderAudioItemThumb(item, i) !== undefined) {

            poolItems.push(_this8.renderAudioItemThumb(item, i));
          }
        });
        return poolItems;
      } else {
